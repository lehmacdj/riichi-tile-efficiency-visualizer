/**
 * Test helpers for Riichi Mahjong algorithms
 *
 * Hand notation format:
 *   - Numbered tiles: digits followed by suit (m=manzu, p=pinzu, s=souzu, z=honors)
 ...
 *   "123m456p789sEEESSh" - Same hand using honor shortcuts (h = end honors)
 ...
 *
 * @example
 * parseHand("123m456p") // Returns tiles for 1m, 2m, 3m, 4p, 5p, 6p
 * parseHand("EEEWWh")   // Returns tiles for 1z, 1z, 1z, 3z, 3z (h ends honors)
 * parseHand("123m4z")   // Mix of numbered and z notation
 */
export function parseHand(notation: string): TileDefinition[] {
...
export function makeTile(suit: Suit, value: number): TileDefinition {
  return { suit, value, str: `${value}${suit}` };
}

/** Create a wall state from hand (removes hand tiles from initial wall) */
export function createWallFromHand(hand: TileDefinition[]): WallState {
...
/** Format tiles back to notation string for debugging */
export function formatHand(tiles: TileDefinition[]): string {
...
/** Parse notation and return array of tile strings like ["1m", "4m", "7p"] */
export function parseTileStrings(notation: string): string[] {
